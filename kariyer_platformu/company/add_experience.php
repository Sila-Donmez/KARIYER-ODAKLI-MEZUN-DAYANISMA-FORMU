...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['company_name'])) {
    ...
    // İŞLEM BİTTİ, PROFİLE GERİ DÖN
    header("Location: ../profile.php"); // YOL DÜZELTİLDİ
    exit();
}
?>
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/profile.css"> <!-- Yeni form css -->
</head>
<body>
    ...
    <script>
        // "Halen çalışıyorum" işaretlenirse Bitiş Tarihi kutusunu gizle
        document.getElementById('c').onchange = function() {
            document.getElementById('e_box').style.display = this.checked ? 'none' : 'block';
            if(this.checked) {
